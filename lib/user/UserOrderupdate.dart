...
class UserOrderupdate extends StatefulWidget {
  UserOrderupdate({this.id,this.CatId,this.SubCatId,this.CityId,this.DisId,this.Images,this.priceFrom,this.priceTo,this.title,this.descrtiption,this.CityName,this.DisName});
  final int id,CatId,SubCatId,CityId,DisId;
  final List Images;
  final String priceFrom,priceTo,title,descrtiption,CityName,DisName;
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return UserOrderupdateStat();
  }
}

class UserOrderupdateStat extends State<UserOrderupdate> {
 ...
 // int SubCatId;
  ...
  void initState() {
    ...
    getCity();
    getCat();
    _controller3.text = "${widget.title}";
    _controller4.text = "${widget.descrtiption}";
    _controller1.text = "${widget.priceFrom}";
    _controller2.text = "${widget.priceTo}";
    _value1 = widget.CityName;
    _value2 = widget.DisName;
...
//            maxLength: 40,
                             ...
                             if (_controller3.text.isEmpty) {
                              ...
                              onBackPress(context, "المبلغ المبدئي أقل من المبلغ النهائي");
                               ...
                               if(ImageFiles.isEmpty){
                                 String Res = await EditOrder2(widget.id,context,CatID,SubCatID,_controller3.text,_controller4.text,CityId,DicId,_controller1.text,_controller2.text);
                                 if(Res == 'Success'){
                                 ...
                                 String Res = await EditOrder(widget.id,ImageFiles, context,CatID,SubCatID,_controller3.text,_controller4.text,CityId,DicId,_controller1.text,_controller2.text);
                                 if(Res == 'Success'){
                                   Navigator.pop(context);
                                   Navigator.pushReplacement(
                                       context,
  ...
  void getCat() async {
    ...
    setState(() {
      ...
      int postion = CatIds.indexOf(widget.CatId);
